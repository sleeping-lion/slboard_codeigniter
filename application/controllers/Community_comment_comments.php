...
class Community_comment_comments extends SL_Controller {
	...
	public function add() {
		...
		if ($this -> form_validation -> run() == TRUE) {
			...
			if ($id = $this -> Community_comment_comment -> insert($data)) {								
				...
				$dd = $this -> Community_comment -> get_content($data['poll_community_comment_id']);

				$this -> load -> model('Community');
				$this -> Community -> update_comment_count_plus($dd['poll_community_id']);
				...
				redirect('communities/view/' . $dd['poll_community_id']);
			} else {
	...
	public function delete($id) {
		...
		if ($this -> Community_comment_comment -> delete($data['id'])) {
			...
			$this -> load -> model('Community');
			$this -> Community -> update_comment_count_minus($dd['poll_community_id']);
